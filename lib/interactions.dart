...
class Interactions {
  ...
  static SlashCommandBuilder get hello {
    return SlashCommandBuilder('hello', '跟你打招呼', [],
  ...
  static SlashCommandBuilder get info {
    return SlashCommandBuilder('info', '查看此機器人的資訊', [],
        guild: rpmtwDiscordServerID)
      ..registerHandler((event) async {
        try {
  ...
  static SlashCommandBuilder get chefRank {
    return SlashCommandBuilder('chef-rank', '看看誰最電！ (前 10 名)', [],
        guild: rpmtwDiscordServerID)
      ..registerHandler((event) async {
        try {
          final Box box = Data.chefBox;
  ...
  static SlashCommandBuilder get covid_19 {
    return SlashCommandBuilder('covid19', '查看今日台灣新冠肺炎疫情資訊', [],
        guild: rpmtwDiscordServerID)
      ..registerHandler((event) async {
        try {
          await event.acknowledge();
  ...
  static SlashCommandBuilder get join {
    return SlashCommandBuilder('join', '讓我進入您的語音頻道', [],
        guild: rpmtwDiscordServerID)
      ..registerHandler((event) async {
        try {
          await event.acknowledge();
      ...
      ..registerHandler((event) async {
        try {
          ...
          final MusicSearchPlatform platform =
              MusicSearchPlatform.values.firstWhere((e) => e.id == platformId);
          ...
          if (result.isPlaylist) {
            infos
                .map((e) => e.identifier)
                .forEach(MusicHandler.playByIdentifier);
            ...
            for (final ITrackInfo info in infos) {
              ...
              final String title =
                  String.fromCharCodes(titleCodeUnits.take(96)) +
  ...
  static SlashCommandBuilder get leave {
    return SlashCommandBuilder('leave', '離開語音頻道並結束播放歌曲', [],
        guild: rpmtwDiscordServerID)
      ..registerHandler((event) async {
        try {
          await event.acknowledge();
  ...
  static SlashCommandBuilder get pause {
    return SlashCommandBuilder('pause', '暫停播放音樂', [],
        guild: rpmtwDiscordServerID)
      ..registerHandler((event) async {
        try {
          await event.acknowledge();
  ...
  static SlashCommandBuilder get resume {
    return SlashCommandBuilder('resume', '繼續播放音樂', [],
        guild: rpmtwDiscordServerID)
      ..registerHandler((event) async {
        try {
          await event.acknowledge();
  ...
  static SlashCommandBuilder get nowPlaying {
    return SlashCommandBuilder('np', '查看正在播放的歌曲', [],
        guild: rpmtwDiscordServerID)
      ..registerHandler((event) async {
        try {
  ...
  static SlashCommandBuilder get queue {
    return SlashCommandBuilder('queue', '查看歌曲隊列', [],
        guild: rpmtwDiscordServerID)
      ..registerHandler((event) async {
        try {
  ...
  static SlashCommandBuilder get skip {
    return SlashCommandBuilder('skip', '跳過並播放下首歌曲', [],
        guild: rpmtwDiscordServerID)
      ..registerHandler((event) async {
        try {
          final IMember member = event.interaction.memberAuthor!;
